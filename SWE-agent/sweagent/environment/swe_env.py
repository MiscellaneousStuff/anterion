...
# Output obs and act
def send_data(txt, suffix):
    logger.info(txt)

    url = f"http://localhost:3000/{suffix}"
    payload = json.dumps({
        "session_id": "test",
        "message": txt
    })
...
class SWEEnv(gym.Env):
    ...
    def __init__(self, args: EnvironmentArguments):
        ...
        self.container_path = args.container_path

        # self.is_from_github_url = is_from_github_url(args.data_path)
        self.is_from_github_url = True
    ...
    def reset(self, index: int = None, apply_test_patch: bool = False) -> Tuple[str, dict]:
        ...
        * Check out base commit
        ...
        # Switch dir to one which we can mount volume to
        # and then verify current path
        self.communicate("mkdir -p /usr/app")
        self.communicate("cd /usr/app")
        # self.communicate("rm -rf *.*")
        # self.communicate("pwd")

        # Install netlify-py
        self.communicate("pip install netlify-py")
        self.communicate("pip install python-dotenv")

        # Write deploy netlify script here
        cmd = f"""echo '{DEPLOY_NETLIFY}' >> deploy_netlify.py"""
    ...
    def communicate(
        ...
        if input.strip() != "exit":
            ...
            # Send the act and then the obs
            send_data(input.strip(), "agent-cli-act")
            send_data(output.strip(), "agent-cli-obs")

...
def read_with_timeout(container, pid_func, timeout_duration):
    ...
    buf_dec = buffer.decode()
    # print(">>> BUF DEF:", buf_dec)
    return buf_dec
...
def _get_non_persistent_container(ctr_name: str, image_name: str, host_path: str, container_path: str) -> Tuple[subprocess.Popen, set]:
    ...
    try:
        with timeout(seconds=2):
            ...
            if output:
                print(f"Unexpected container setup output: {output}")  # Use logger.error if logging is set up
    except TimeoutError:
        pass
...
def get_container(ctr_name: str, image_name: str, host_path: str, container_path: str, persistent: bool = False) -> subprocess.Popen:
    """
